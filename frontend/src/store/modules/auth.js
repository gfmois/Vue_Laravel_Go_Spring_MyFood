        ...
        [Constant.CHECK_IS_ADMIN]: (store, payload) => {
            AuthService.checkAdmin().then(res => {
                if (res.data.status == "success") {
                    store.commit(Constant.CHECK_IS_ADMIN, res.data.message.isAdmin)
                }
            })
            .catch(error => {
                store.commit(Constant.CHANGE_HAS_USER, false)
                router.push("/auth")
            })
        },
        [Constant.CHECK_HAS_USER]: (store, payload) => {
            cookies.get("token_client") ? store.commit(Constant.CHECK_HAS_USER, true) : store.commit(Constant.CHECK_HAS_USER, false)
        }, 
